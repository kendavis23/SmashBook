...
**Offset-aware in (normalized to UTC at the boundary), UTC in storage, club-local on the way out. The frontend just displays what it receives.** Incoming datetimes must carry an explicit offset — the client may send the user's local time so long as it is offset-tagged (`...+02:00`); a naive, offset-less value is rejected (`422`), never silently interpreted. Follow this without exception — the two halves of the codebase used to disagree on what a stored `start_datetime` meant, and the operational booking/availability path silently treated club-local wall-clock time as UTC.
...
- **Wire — parse-in:** every incoming datetime (request bodies *and* datetime query-param filters) is typed `UtcDatetime` (`app/schemas/common.py`): naive (offset-less) values are **rejected at the schema layer** (`422`), and offset-bearing values are normalized to UTC. The frontend may send any valid offset; the backend converts it.
