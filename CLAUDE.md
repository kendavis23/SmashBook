...
**Naive-or-offset in (normalized to UTC at the boundary), UTC in storage, club-local on the way out. The frontend just displays what it receives.** The frontend is **never required** to attach a UTC offset. A naive (offset-less) incoming datetime is accepted and interpreted by scope; an offset-bearing value is honored and converted to UTC. What is forbidden is the old bug: silently stamping a club-local wall-clock value as UTC and keeping the wrong number. Follow this without exception — the two halves of the codebase used to disagree on what a stored `start_datetime` meant, and the operational booking/availability path silently treated club-local wall-clock time as UTC.

> **Forward-looking mandate — applies to ALL new datetime work.** Every new endpoint, schema field, or datetime query-param MUST follow this pattern: pick the right parse-in type, convert at the boundary through `app/core/timezones.py`, store UTC, render club-local. **Never** add an offset-required (`422`-on-naive) input, and **never** stamp a wall-clock value as UTC with `datetime.combine(..., tzinfo=timezone.utc)` / `.replace(tzinfo=timezone.utc)` outside the sanctioned helpers. No exceptions without updating this section.
...
- **Wire — parse-in:** every incoming datetime is typed via `app/schemas/common.py` and normalized to UTC at the boundary. Naive values are **accepted** (no `422`), resolved by scope:
  - **Club-scoped** inputs (request bodies *and* datetime query-param filters that carry a `club_id`) use `ClubLocalDatetime`: offset-aware → UTC at the schema layer; naive → passed through, then interpreted as **club-local wall-clock** and converted in the endpoint via `ensure_utc(dt, club_tz(club))`. Every such endpoint already resolves the club tz for render-out (`_resolve_club_tz` / `club_tz`); localize the input with the same tz before calling the service.
  - **Non-club-scoped** inputs (tenant/billing — no single club zone) use `UtcCoercedDatetime`: offset-aware → UTC; naive → **stamped UTC**. Today the only such input is `subscription_start_date` (a billing anchor). Revisit (e.g. add `tenants.timezone`) when a real tenant-level wall-clock input lands.
  - **`date`-typed** calendar inputs (`date_from`/`date_to`, `valid_from`, `effective_from`, etc.) need no conversion — a bare date carries no offset and is already a club-local calendar day.
- **Wire — render-out:** club-scoped read endpoints emit **club-local tz-aware** datetimes (offset such as `+02:00`, never bare `Z`), converted via `utc_to_local`. The instant stays unambiguous via the offset while the wall-clock reads local; the frontend displays it verbatim. **Audit/billing timestamps stay UTC** (`created_at`, `updated_at`, Stripe/subscription periods): they are tenant/platform-level with no single club zone, so a single response legitimately mixes offsets. Do **not** "correct" the club-local render-out back to UTC.
- **Server-generated `now()`:** always `datetime.now(tz=timezone.utc)` (never naive `datetime.utcnow()`).
- **Conversion:** the *only* sanctioned local↔UTC conversion path is `app/core/timezones.py` (`club_tz`, `local_walltime_to_utc`, `utc_to_local`, and `ensure_utc` for parse-in of incoming club-scoped datetimes), resolving local time via the `clubs.timezone` IANA column. **Never** write `datetime.combine(d, t, tzinfo=timezone.utc)` for a club-local wall-clock value — build it in the club's zone and convert.
...
Defined in `backend/app/core/config.py` via pydantic-settings. For local dev, create `backend/.env` (gitignored). Required: `SECRET_KEY`, `DATABASE_URL`, `DATABASE_READ_REPLICA_URL`, `PUBSUB_PROJECT_ID`, `GCS_BUCKET_VIDEOS`, `GCS_BUCKET_INVOICES`, `GCS_BUCKET_EXPORTS`, `GCS_PROJECT_ID`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_CONNECT_WEBHOOK_SECRET`, `STRIPE_BILLING_SECRET_KEY`, `STRIPE_BILLING_WEBHOOK_SECRET`, `PLATFORM_API_KEY`, `SENDGRID_API_KEY`, `FIREBASE_PROJECT_ID`, `APP_BASE_URL` (e.g. `https://smashbook.app`; used to construct password-reset links; defaults to that value if unset).
