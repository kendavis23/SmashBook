...
variable "player_domain" {
  description = "Full FQDN for the player portal (e.g. app.smashbook.app). Leave empty to skip creating player DNS records."
  type        = string
  default     = ""
}
...
resource "cloudflare_record" "staff_a" {
  ...
  type    = "A"
  value   = data.terraform_remote_state.gcp_prod.outputs.lb_static_ip_staff
  proxied = true
}
...
resource "cloudflare_record" "player_a" {
  ...
  type    = "A"
  value   = data.terraform_remote_state.gcp_prod.outputs.lb_static_ip_player
  proxied = true
}
