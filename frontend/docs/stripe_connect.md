...
*The frontend never talks directly to your backend to confirm a payment.* Instead it talks directly to Stripe using Stripe.js, and your backend gets notified afterwards via webhook. This is critical — the webhook (payment_intent.succeeded) is the sole source of truth for whether a booking is paid.
...
This uses a *SetupIntent* instead of a PaymentIntent — it authorises storing the card without charging it.
