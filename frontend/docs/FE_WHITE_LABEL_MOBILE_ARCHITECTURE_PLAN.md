...
- Run the full build → submit → OTA loop once, manually.
- **Exit criterion:** two distinct branded apps from one codebase. Document every manual step — this becomes the runbook.

### Phase 5 — Feature flags + Model B (shared app)
- Layered flag resolution (`useBrandFlags()`), backend player-flag + brand-resolution endpoints.
- Shared-app flow: pick a club → re-skin at runtime.
- **Exit criterion:** a new club can be added to the shared app with zero rebuild.

### Phase 6 — Asset & build automation (scale)
- Asset generation pipeline (one master logo → all sizes).
- CI brand-matrix build orchestration; OTA channels per brand; rollback runbook.
- Brand-kit intake spec for client onboarding.
- **Exit criterion:** adding a dedicated brand is a manifest + asset master + a pipeline run — no bespoke engineering.
