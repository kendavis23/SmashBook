...
import { coachDisplayName, panelCls, panelTitleCls } from "../coachPopularityConstants";
...
type Props = {
    title: string;
    hint: string;
    /** Column header for the right-hand metric (e.g. "Sessions"). */
    metricLabel: string;
    rows: CoachPopularityRow[];
    /** Maps a row to its metric. */
    metricOf: (row: CoachPopularityRow) => MetricCell;
    active?: boolean;
    onViewAll: () => void;
};

/**
 * A compact top-5 leaderboard card: rank, coach name, and one metric.
 */
export function CoachLeaderboardPanel({
    title,
    hint,
    metricLabel,
    rows,
