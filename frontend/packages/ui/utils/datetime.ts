...
/**
 * Converts a datetime-local input value ("YYYY-MM-DDTHH:mm") to the wall-clock
 * datetime string the API expects ("YYYY-MM-DDTHH:mm:00") — no timezone marker.
 * The backend applies its own timezone handling, so we send the time as-is and
 * never append a "Z". Use this instead of `new Date(value).toISOString()`, which
 * would shift by the local offset and append "Z".
 */
