...
#### Materialized view: `mv_coach_popularity`
Backs the "Coach popularity" report (G7). **Materialized view, not an ORM table** — created by hand-written DDL in migration `1f0263eec44b`, refreshed nightly by `app/analytics/workers/refresh_views.py`. Grain: one row per `(club_id, staff_profile_id)` — a point-in-time stock. A coaching session is a lesson booking (`booking_type IN (lesson_individual, lesson_group, train_and_play)`) with a non-null `staff_profile_id`, non-cancelled + already-started. Columns: `sessions`, `first_session_at`, `last_session_at`, `sessions_last_30d`, `sessions_last_90d`, `distinct_players`, `repeat_players` (players with ≥2 lessons with this coach), `total_attendances`, `lesson_revenue` (net, `payments` by `booking_id`), `currency`. `return_rate` (= `repeat_players / distinct_players`) is derived in the service, not stored. Coach display names join `users` via `staff_profiles` live (no PII in the view). `UNIQUE (club_id, staff_profile_id)` (required for `REFRESH … CONCURRENTLY`). Full spec: [`REPORT_CATALOG.md`](../backend/app/analytics/docs/REPORT_CATALOG.md) and [`MATERIALIZED_VIEWS.md`](../backend/app/analytics/docs/MATERIALIZED_VIEWS.md).

#### Materialized view: `mv_player_rfv`
The RFV (Recency / Frequency / Value) **pre-aggregate** (G7). **Materialized view, not an ORM table** — created by hand-written DDL in migration `dff4cf6de626`, built **on top of** `mv_player_value` (must refresh after it; registered immediately after it in `REFRESH_VIEWS`). Grain: one row per `(club_id, user_id)`, restricted to *engaged* players (`bookings_played > 0 OR lifetime_spend > 0`). Scores each on Recency (`last_played_at`), Frequency (`bookings_played`), and Value (`lifetime_spend`) via per-club `ntile(5)` quintiles (1 = lowest .. 5 = highest). Columns: `recency_score`, `frequency_score`, `value_score`, `rfv_total` (sum), `rfv_cell` (e.g. `"543"`) plus the raw inputs. **A pre-aggregate only — no named-segment taxonomy** (structured segmentation deferred; see the dropped `player_segments` decision). `UNIQUE (club_id, user_id)` (required for `REFRESH … CONCURRENTLY`). Full spec: [`MATERIALIZED_VIEWS.md`](../backend/app/analytics/docs/MATERIALIZED_VIEWS.md).
