...
RETRYABLE = (OSError, urllib.error.URLError)  # covers SSLEOFError, connection resets

def rest(token, method, path, body=None, _retries=3):
    ...
    for attempt in range(1, _retries + 1):
        try:
            ...
            if attempt == _retries:
                print(f"  Network error after {_retries} attempts: {e}")
                raise
            wait = 2 ** attempt  # 2s, 4s, 8s
            print(f"  Network error (attempt {attempt}/{_retries}), retrying in {wait}s: {e}")
            time.sleep(wait)


def graphql(token, query, variables=None, _retries=3):
...
def get_existing_issue_titles(token, owner, repo):
    """Fetch all open+closed issue titles so we can skip already-created ones on re-run."""
    titles = set()
    page = 1
    while True:
        issues = rest(token, "GET",
...
def get_or_find_project(token, user_id, login, title):
    """Return existing project with matching title, or create a new one."""
    d = graphql(token, """
        query($login: String!) {
            user(login: $login) {
                projectsV2(first: 20) {
    ...
    for p in d["user"]["projectsV2"]["nodes"]:
        if p["title"] == title:
            print(f"  Found existing project: {p['url']}")
            return p["id"], p["number"], p["url"]
    return create_project(token, user_id, title)
...
def main():
    ...
    # ── Project ───────────────────────────────────────────────────────────────
    print(f"\nFinding or creating GitHub Project: '{args.project}'...")
    proj_id, proj_num, proj_url = get_or_find_project(args.token, user_id, login, args.project)
    print(f"  Project ready: {proj_url}")

    # ── Fetch existing issues (for resume support) ────────────────────────────
    print("\nChecking for existing issues (to skip duplicates on re-run)...")
    existing_titles = get_existing_issue_titles(args.token, args.owner, args.repo)
    skipped = sum(
        1 for s in SPRINTS for _, story in s["stories"] if story in existing_titles
    )
    if skipped:
        print(f"  {skipped} issues already exist — will skip them and resume from where we left off")
    else:
        print(f"  No existing issues found — starting fresh")
    ...
    for sprint in SPRINTS:
        ...
        for area, story in sprint["stories"]:
            # Skip if already created (safe re-run)
            if story in existing_titles:
                skipped_count += 1
                print(f"    [skip] Already exists: {story[:60]}...")
                continue
            ...
            created += 1
            existing_titles.add(story)  # prevent dupes within this run

            # Add to project
            node_id = get_issue_node_id(args.token, args.owner, args.repo, issue_num)
            add_issue_to_project(args.token, proj_id, node_id)

            print(f"    [{created} new] #{issue_num}: {story[:58]}...")
            time.sleep(0.3)  # respect secondary rate limits

        print()

    print(f"Done — {created} issues created, {skipped_count} skipped (already existed).")
