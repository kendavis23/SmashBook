...
locals {
  pubsub_topics = [
    ...
    "analytics-alerts",
    # Stage 2.6 — daily wallet-debt settlement trigger. Cloud Scheduler publishes
...
resource "google_pubsub_subscription" "wallet_settlement_events" {
  name  = "wallet-settlement-events-sub"
  topic = google_pubsub_topic.topics["wallet-settlement-events"].name

  push_config {
    push_endpoint = "${var.settlement_worker_uri}/pubsub"
  ...
  # Settlement issues Stripe Connect transfers; give it a generous ack window.
  # Redelivery is safe (deterministic Stripe idempotency keys + settled_at drops
  # debts out of the query), so DLQ only catches genuine poison messages.
  ack_deadline_seconds       = 600
  message_retention_duration = "604800s"
  retain_acked_messages      = false
  ...
  dead_letter_policy {
    dead_letter_topic     = google_pubsub_topic.wallet_settlement_events_dlq.id
    max_delivery_attempts = 5
  }
}
...
resource "google_cloud_run_v2_service_iam_member" "pubsub_invoke_settlement" {
  project  = var.project_id
  location = var.region
