terraform {
  ...
  # TODO: create this GCS bucket in the prod project before running terraform init.
  # gcloud storage buckets create gs://tf-state-<prod-project-id>-backend --location=europe-west2
  backend "gcs" {
    bucket = "tf-state-smashbook-prod-REPLACE_ME-backend"
    prefix = "prod"
  }
}
...
module "scheduler" {
  ...
  # Daily payout reconciliation (04:00 UTC) — ships PAUSED. Flip
  # payout_reconcile_paused to false (here) to enable the automated sweep.
  payout_reconcile_events_topic_id = module.pubsub.payout_reconciliation_events_topic_id
  payout_reconcile_paused          = true
}
